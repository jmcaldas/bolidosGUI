...
% Last Modified by GUIDE v2.5 02-Feb-2021 09:57:56
...
if strcmp(status,'RUN')    
    ...
    rmT = str2double(get(handles.edit16,'String'));
        ...
        while flagStop            
            if 90-sun.zenith<sun_el_max
               ...
               if FN < thrFN
                   ...
                   while nDif>=nThr && nFr<250 && rm<=rmT
                       detection=1;
                       im1=getsnapshot(vid);                       
                       pause(0.1);
                       im2=getsnapshot(vid);
                   ...
                   if detection
                       ...
                       pause(1); 
                       stop(vid);
                       % Get delta time between GPS and PC
                       try
                       ...
                       try
                           ...
                           % to first frame. It is NOT GPS time. Time shift 
                           % between GPS and PC  is saved in .mat file, along
                           % with a flag indicating whether GPS was idle
...
try
    ...
    diff=zeros(size(im1));
    count=0;
    for i=4:3:150 % ca 100 ms between frames        
        im2=frames(:,:,1,i);
        im2=imresize(im2,resolucion);
        im2=im2.*uint8(mascara);
        
        diff=double(imabsdiff(im1,im2))+diff;        
        count=count+1;
        im1=im2;
    end
    diff=diff./count; %Mean difference image
    
    figure; histogram(diff(:));title('Histograma de imagen diferencia promedio');
    unfreeze(handles);    
    set(handles.edit1,'String',num2str(max(diff(:)),'%.0f'));
    set(handles.text12,'String','Listo!');
catch err
    set(handles.text12,'String',strcat('Error:',err.message));
end
